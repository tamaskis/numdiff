use crate::constants::FOURTH_ROOT_EPS;

/// Second derivative of a univariate, scalar-valued function using the central difference
/// approximation.
///
/// # Arguments
///
...
/// * `h` - Relative step size, $h\in\mathbb{R}$. Defaults to [`FOURTH_ROOT_EPS`].
///
/// # Returns
///
...
/// use numtest::*;
///
/// use numdiff::central_difference::sderivative2;
///
...
/// assert_equal_to_decimal!(d2f, 12.0, 6);
/// ```
///
...
mod tests {
    ...
    fn test_product_rule() {
        ...
        assert_equal_to_decimal!(d2h_approx(-1.5), d2h(-1.5), 5);
        assert_equal_to_decimal!(d2h_approx(1.5), d2h(1.5), 5);
    }
    ...
    fn test_sderivative2_polynomial() {
        ...
        assert_equal_to_decimal!(sderivative2(&f, -2.0, None), d2f(-2.0), 5);
        assert_equal_to_decimal!(sderivative2(&f, 2.0, None), d2f(2.0), 5);
    }
    ...
    fn test_sderivative2_inverse_secant() {
        let f = |x: f64| x.asec();
        assert_equal_to_decimal!(sderivative2(&f, 1.5, None), test_utils::asec_deriv2(1.5), 5);
        assert_equal_to_decimal!(
    ...
    fn test_sderivative2_inverse_cosecant() {
        let f = |x: f64| x.acsc();
        assert_equal_to_decimal!(sderivative2(&f, 1.5, None), test_utils::acsc_deriv2(1.5), 5);
        assert_equal_to_decimal!(
            sderivative2(&f, -1.5, None),
