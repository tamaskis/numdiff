...
/// Second derivative of a univariate, vector-valued function using the central difference
/// approximation.
///
/// # Arguments
///
...
/// use numtest::*;
///
/// use numdiff::central_difference::vderivative2;
///
...
/// assert_arrays_equal_to_decimal!(d2f, d2f_true, 5);
/// ```
///
...
/// assert_arrays_equal_to_decimal!(d2f_at_1, d2f_at_1_true, 5);
/// ```
    ...
    V: Vector<f64>,
{
    ...
    (f(x0 + dx).sub(&f(x0).mul(2.0)).add(&f(x0 - dx))).div(dx.powi(2))
}
...
mod tests {
    ...
    fn test_vderivative2() {
        ...
        assert_arrays_equal_to_decimal!(vderivative2(&f, x0, None), d2f(x0), 5);
    }
