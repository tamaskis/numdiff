...
/// use numtest::*;
///
/// use numdiff::central_difference::mixed_spartial_derivative2;
///
...
/// assert_equal_to_decimal!(d2f, 8.0, 5);
/// ```
...
mod tests {
    ...
    fn test_mixed_spartial_derivative2_basic() {
        let f = |x: &Vec<f64>| x[0].powi(4) + 2.0 * x[0].powi(2) * x[1] + x[1].powi(3);
        let x0 = vec![2.0, 1.0];
        assert_equal_to_decimal!(mixed_spartial_derivative2(&f, &x0, 0, 1, None), 8.0, 5);
        assert_equal_to_decimal!(mixed_spartial_derivative2(&f, &x0, 1, 0, None), 8.0, 5);
    }
    ...
    fn test_mixed_spartial_derivative2_same_index() {
        ...
        let f = |x: &Vec<f64>| x[0].powi(4) + 2.0 * x[0].powi(2) * x[1] + x[1].powi(3);
        let x0 = vec![2.0, 1.0];
        assert_equal_to_decimal!(mixed_spartial_derivative2(&f, &x0, 0, 0, None), 52.0, 3);
