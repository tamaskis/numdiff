...
/// Second-order partial derivative of a multivariate, scalar-valued function using the central
...
/// use numtest::*;
///
/// use numdiff::central_difference::spartial_derivative2;
///
...
/// assert_equal_to_decimal!(d2f, 52.0, 4);
/// ```
///
...
/// assert_equal_to_decimal!(d2f_dx0dx0, 2.0 * a, 4);
/// ```
    ...
    V: Vector<f64>,
{
    ...
    // Evaluate at the original evaluation point.
    x0[k] = x0k;
    let f0 = f(&x0);
...
mod tests {
    ...
    fn test_spartial_derivative2_1() {
        let f = |x: &Vec<f64>| x[0].powi(4) + 2.0 * x[0].powi(2) * x[1] + x[1].powi(3);
        let x0 = vec![2.0, 1.0];
        assert_equal_to_decimal!(spartial_derivative2(&f, &x0, 0, None), 52.0, 4);
        assert_equal_to_decimal!(spartial_derivative2(&f, &x0, 1, None), 6.0 * x0[1], 5);
    }
