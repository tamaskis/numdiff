...
/// Second-order partial derivative of a multivariate, vector-valued function using the central
...
/// use numtest::*;
///
/// use numdiff::central_difference::vpartial_derivative2;
///
...
/// assert_arrays_equal_to_decimal!(d2f, vec![52.0, 0.0], 4);
/// ```
    ...
    U: Vector<f64>,
{
    ...
    (f1.sub(&f0.mul(2.0)).add(&f2)).div(dxk.powi(2))
}
...
mod tests {
    ...
    fn test_vpartial_derivative2() {
        ...
        let x0 = vec![2.0, 1.0];
        assert_arrays_equal_to_decimal!(vpartial_derivative2(&f, &x0, 0, None), [52.0, 0.0], 4);
